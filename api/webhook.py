...
def send_city_buttons(chat_id):
    # Use Reply Keyboard — buttons stretch full width like the reference bot
    rows = []
    for group_name, cities in CITY_GROUPS.items():
        # Group header as a non-city label row
    ...
    # Store that we're waiting for a city via reply keyboard
...
def handle_message(message):
    ...
    # Handle city selection from Reply Keyboard
    state = user_state.get(user_id, {})
    if state.get("step") == "waiting_city" and user_id in validated_users:
        # Check if text matches a city (strip the time part if present)
        city_match = None
        for city in CITIES:
            if text.startswith(city) or text == city:
                city_match = city
                break
        if city_match:
            user_state[user_id] = {"step": "waiting_date", "city": city_match}
            send_date_buttons(chat_id, city_match)
            return
        # If it's a group header tap, ignore
