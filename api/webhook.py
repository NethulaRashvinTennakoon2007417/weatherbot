...
def send_city_buttons(chat_id):
    rows = []
    # Add a wide invisible spacer button to force full width on all buttons
    for group_name, cities in CITY_GROUPS.items():
        rows.append([{"text": f"　　　── {group_name} ──　　　", "callback_data": "noop"}])
        for city in cities:
            t = get_city_time(city)
            # Pad with ideographic spaces to stretch button width
            label = f"　　{city}   🕐 {t}　　" if t else f"　　{city}　　"
