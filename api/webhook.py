...
All fixes applied:
- After result: buttons to check another date (same city) or new city
- Wide single-column buttons for cities and dates
- City-local date calculation
- Proper state management
- Clean grouped city list
- Auth error handling
"""
...
# Persistent Redis storage — survives Vercel cold starts
redis = Redis.from_env()

def get_user_code(user_id: int) -> str:
    try:
        return redis.get(f"we:code:{user_id}") or ""
    except:
        return ""

def set_user_code(user_id: int, code: str):
    try:
        redis.set(f"we:code:{user_id}", code)  # persistent, no expiry
    except:
        pass

def del_user_code(user_id: int):
    try:
        redis.delete(f"we:code:{user_id}")
    except:
        pass

def get_user_state(user_id: int) -> dict:
    try:
        val = redis.get(f"we:state:{user_id}")
...
def set_user_state(user_id: int, state: dict):
    try:
        redis.set(f"we:state:{user_id}", json.dumps(state), ex=3600)  # 1hr expiry
    except:
        pass

def del_user_state(user_id: int):
    try:
        redis.delete(f"we:state:{user_id}")
    except:
        pass
...
CITY_GROUPS = {
    ...
    "🌐 International":   ["Seoul", "Buenos Aires"],
}
...
CITY_TIMEZONES = {
    ...
    "New York (LaGuardia)": "America/New_York",
...
def validate_code(code):
    try:
        r = req.post(f"{VERCEL_URL}/api/validate_code", json={"code": code}, timeout=15)
        return r.json()
    except Exception as e:
        return {"ok": False, "error": str(e)}

# Map bot city names to exact API city names used in WeatherEdge
CITY_API_NAMES = {
...
def get_weather(city, date_str, code):
    try:
        # Use the exact API name, encode spaces as %20
...
def handle_message(message):
    ...
    if text.startswith("/start"):
        if get_user_code(user_id):
            ...
            set_user_state(user_id, {"step": "waiting_code"})
            send_message(chat_id,
    ...
    if text.startswith("/analyze"):
        if not get_user_code(user_id):
            ...
            return
        set_user_state(user_id, {"step": "waiting_city"})
...
def handle_callback(callback):
    ...
    if data.startswith("city:"):
        city = data[5:]
        set_user_state(user_id, {"step": "waiting_date", "city": city})
    ...
    if data.startswith("date:"):
        ...
        # Keep city in state for post-result buttons
        set_user_state(user_id, {"step": "has_result", "city": city})
