...
# Cities grouped by category — matches WeatherEdge exactly
CITY_GROUPS = {
...
def get_city_time(city: str) -> str:
    """Return current local time for a city as HH:MM string."""
    try:
...
def send_city_buttons(chat_id):
    ...
    for group_name, cities in CITY_GROUPS.items():
        # Group header as a disabled button
        rows.append([{"text": f"── {group_name} ──", "callback_data": "noop"}])
        # One city per row with local time
        for city in cities:
            local_time = get_city_time(city)
            label = f"{city}  🕐 {local_time}" if local_time else city
...
def send_date_buttons(chat_id, city):
    # Use the city's LOCAL date — not the server's date
    try:
        tz = ZoneInfo(CITY_TIMEZONES.get(city, "UTC"))
...
def handle_callback(callback):
    ...
    if data == "noop":
        return  # group header tapped, do nothing
